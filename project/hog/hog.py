...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    n, scores, no_one = 1, 0, False
    while n <= num_rolls:
        score = dice()
        if score == 1:
            no_one = True
        n, scores = n + 1, scores + score
    if no_one == True:
        return 1
    return scores
    # END PROBLEM 1
...
def is_swap(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    return abs(player_score % 10 -
               opponent_score % 10) == opponent_score // 10 % 10
    # END PROBLEM 4
...
def play(strategy0,
    ...
    while True: 
        s = strategy(who)
        op_score = who_score(other(who))
        player_score = who_score(who)
        num_roll = s(player_score,op_score)
        score = take_turn(num_roll,op_score,dice)
        if feral_hogs:
            last_score = last_score0 if who==0 else last_score1
            if abs(last_score-num_roll)==2:
               player_score = player_score +3
        player_score=player_score+score
        if is_swap(player_score,op_score):
            player_score,op_score = op_score,player_score
        if who==0:
            score0 = player_score
            score1 = op_score
            last_score0 = score
